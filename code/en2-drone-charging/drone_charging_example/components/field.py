...
class Field:
    ...
    def __init__(self, pointLists):
        ...
        self.damage = 0
        self.location = Point((self.topLeft.x + self.bottomRight.x) / 2, (self.topLeft.y + self.bottomRight.y) / 2)

        # assert self.topLeft.x + self.droneRadius > self.bottomRight.x
        # assert self.topLeft.y + self.droneRadius > self.bottomRight.y

        def get_ranges():
            """to deal with the situation that field is smaller in one or both dimensions than the drone's radius"""
            a = range(self.topLeft.x + round(self.droneRadius), self.bottomRight.x, round(self.droneRadius))
            b = range(self.topLeft.y + round(self.droneRadius), self.bottomRight.y, round(self.droneRadius))
            if len(a) == 0:
                a = [(self.topLeft.x + self.bottomRight.x) // 2]
            if len(b) == 0:
                b = [(self.topLeft.y + self.bottomRight.y) // 2]
            return a, b
    ...
    def unassign(self, drone, force_removal=True):
        """
        ...
        if drone in self.protectingDrones:
            ...
            if drone.state == DroneState.TERMINATED or force_removal:
