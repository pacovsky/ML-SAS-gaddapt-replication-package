...
class World:
    ...
    def reset(self, args):
        ...
        self.birds: List[Bird] = [Bird(Point.random(0, 0, ENVIRONMENT.mapWidth, ENVIRONMENT.mapHeight))
                                  for _ in range(ENVIRONMENT.birdCount)]
        self.chargers: List[Charger] = [Charger(Point(position), ENVIRONMENT.chargingRate, ENVIRONMENT.chargerCapacity)
                                        for position in ENVIRONMENT.chargerPositions]
        ...
        components = []
        components.extend(WORLD.birds) #TODO this makes birds move before drones
        ...
        if ENVIRONMENT.droneCount > 0:
            ...
            from ensembles.drone_charging import getEnsembles as droneChargingEnsembles
            from ensembles.another_charging import getEnsembles as anotherChargingEnsembles
            if args.which_ensembles == "field+another_charging":
                potentialEnsembles = fieldProtectionEnsembles(args) + anotherChargingEnsembles(args)
            elif args.which_ensembles == "field":
                potentialEnsembles = fieldProtectionEnsembles(args)
            elif args.which_ensembles == "another_charging":
                potentialEnsembles = anotherChargingEnsembles(args)
            elif args.which_ensembles == "minimal_charging":
                potentialEnsembles = droneChargingEnsembles(args)
            elif args.which_ensembles == "field+minimal_charging":
                potentialEnsembles = fieldProtectionEnsembles(args) + droneChargingEnsembles(args)
            else:
                raise RuntimeError(f"unknown value {args.which_ensembles = }")
        else:
            potentialEnsembles = []
